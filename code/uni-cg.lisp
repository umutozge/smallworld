...
(defvar *vocabulary-path*)
(defvar *lexicon-path*)
(defvar *lexical-rules-path*)
...
(defun proc-vocab-item (v)
  (let* ((form (car v))
         (lemma (cadr v))
         (pos (caddr v))
         (base-item (build-entry form lemma pos)))
    (cons base-item
          (remove-if #'null
                     (mapcar
                       #'(lambda (x) (apply-lexical-rule x base-item))
                       *lexical-rules*)))))

(defun generate-lexicon (&key (lex-path *lexicon-path*) (vocab-path *vocabulary-path*))
  "Generates a lexicon (and writes to lex-path) on the basis of a vocabulary (a tab-seperated value file
   located at vocab-path by default) and the lexical templates and rules in lexicon-factory.lisp"
  (let ((vocab-list (aux:tsv-to-list vocab-path)))
    (with-open-file (outstr lex-path
                            :direction :output
                            :if-exists :overwrite
                            :if-does-not-exist :create)
      (format outstr ";;;~%;;; This lexicon file is automatically generated by lexicon-factory.lisp~%~%;;; Manual editing is also possible for experimenting~%;;; but you will need to read-lexicon~%;;; and beware that generate-lexicon will overwrite this file~%~%")
      (dolist (v vocab-list t)
        (format outstr "~{~A~%~}~%" (proc-vocab-item v))))))

(defun read-lexicon (&key (path *lexicon-path*))
  "Reads lexical entries from path, parses them into signs, and stores them in a aux:multi-set-table (see aux.lisp).
   the global var *lexicon* ends up pointing to a closure where one can add, get or 
   query lexical entries using the top level phon feature.
   The closure *lexicon* is a mapping from phon to the list of signs associated with that phon"
...
; (defun parse (sentence)
;   (let ((enumerations (generate-enums sentence))
; 		(store nil))
; 	(funcall (if (not *eta-normalize*) 
; 			   #'identity
; 			   #'(lambda (x)
; 				   (mapcar 
; 					 #'(lambda (y)
...
;   (setf *vocabulary-path* (make-pathname :name (concatenate 'string project-path "/vocabulary.tsv")))
  (setf *lexicon-path* (aux:string-to-pathname project-path "/_lexicon.lisp" ))
...
; 							  (run-program "/usr/bin/which" '("rm") :output out)))
; 			   (list (concatenate 'string (sb-unix::posix-getenv "SW_HOME")
; 								  "/"
; 								  project-path
; 								  "/lexicon.lisp"))
; 			   :output nil)
;   (load (concatenate 'string project-path "/lexicon-factory.lisp"))
;   (read-lexical-rules)
;   (generate-lexicon)

  (lexicon-parser:run project-path)
  (read-lexicon))
